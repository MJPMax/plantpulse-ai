...
export default function TrendChart({ series, height = 260, normalRange, anomalyBands, refLines, title }: Props) {
    ...
    <Box>
      ...
      <ResponsiveContainer width="100%" height={height}>
        <LineChart data={merged} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
          ...
          {refLines?.map((rl, i) => (
            <ReferenceLine key={`ref-${i}`} y={rl.value} stroke={rl.color} strokeDasharray={rl.dashed ? '6 3' : undefined} strokeWidth={1.2}
