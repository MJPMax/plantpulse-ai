...
import {
  ...
  Avatar, Box, IconButton, Card, CardContent, Collapse,
} from '@mui/material';
      ...
      <Card variant="outlined" sx={{ mb: 2, border: '2px solid', borderColor: 'primary.main' }}>
        <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
          ...
          <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 1.5 }}>
            <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>Create Action</Typography>
          ...
          <TextField size="small" fullWidth value={title} onChange={(e) => setTitle(e.target.value)}
            ...
            sx={{ mb: 1.5, '& input': { fontWeight: 600, fontSize: '0.9rem' } }} />

          {/* Assignee */}
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.5, p: 1.25, bgcolor: '#F5F3F0', borderRadius: 1, border: '1px solid #E8E6E3' }}>
            <Avatar sx={{
              ...
              {assigneeName === 'Unassigned' ? <PersonIcon sx={{ fontSize: 18 }} /> : userInitials(assigneeName)}
            </Avatar>
            <Box sx={{ flex: 1 }}>
              ...
              <Select size="small" value={assigneeUserId} displayEmpty
                ...
                sx={{ mt: 0.5, minWidth: 160, fontSize: '0.82rem' }}>
